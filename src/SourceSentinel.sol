...
contract SourceSentinel is Ownable {
    ...
    address public layerTransmitter;

    address public pushChannelAdmin;

    constructor(address _layerTransmitter) Ownable() {
        layerTransmitter = _layerTransmitter;
    }
    ...
    function deposit(uint256 amount, bytes32 recipient) external {
        ...
        // Approve the Wormhole Transceiver to spend the tokens
        require(WETH_NTT.approve(address(WORMHOLE_TRANSCEIVER), amount), "Approve failed");
        ...
        // Call the Wormhole Transceiver to initiate a cross-chain transfer
        WORMHOLE_TRANSCEIVER.transferTokens(
            address(WETH_NTT),
            amount,
